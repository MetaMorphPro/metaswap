...
contract MetmMaker {
    ...
    constructor(IUniswapV2Factory _factory, address _bar, address _meta, address _weth) public {
        factory = _factory;
        meta = _meta;
        bar = _bar;
        weth = _weth;
    ...
    function _toWETH(address token) internal returns (uint256) {
        ...
        if (token == weth) {
            uint amount = IERC20(token).balanceOf(address(this));
            IERC20(token).transfer(factory.getPair(weth, meta), amount);
            return amount;
        }
        ...
        pair.swap(amount0Out, amount1Out, factory.getPair(weth, meta), new bytes(0));
        return amountOut;
    }
